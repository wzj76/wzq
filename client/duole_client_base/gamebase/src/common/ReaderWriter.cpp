...
int ReaderWriter::LoadIntegerFromXML(const string &key, int default_value)
{
    ...
    const string kEncodedDefaultValue = Encode(flatbuffers::NumToString(default_value));
...
{
    SaveStringForKey(Encode(key), Encode(flatbuffers::NumToString(value)));
}
...
void ReaderWriter::SaveFloatFromXML(const string &key, float value)
{
    SaveStringForKey(Encode(key), Encode(flatbuffers::NumToString(value)));
}
