...
class AndroidBuilder(object):
    ...
    def update_project(self, android_platform):
        ...
        android_app_path = os.path.join(self.app_android_root, 'app')

        # check the android platform
        # target_str = self.check_android_platform(self.sdk_root, android_platform, manifest_path)
        target_str = self.check_android_platform(self.sdk_root, android_platform, android_app_path)
        ...
        self._write_local_properties(android_app_path)
        self._update_project_properties(android_app_path, target_str)

        # copy the local.properties to the app_android_root
        file_name = 'local.properties'
        # src_path = os.path.normpath(os.path.join(manifest_path, file_name))
        src_path = os.path.normpath(os.path.join(android_app_path, file_name))
    ...
    def gradle_build_apk(self, channel, channel_group, mode, android_platform, compile_obj):
        ...
        # by zhangjm: channel
        # -PANDROID_CHANNEL_GROUP=duole -PANDROID_CHANNEL=duole
        channel_str = '-PANDROID_CHANNEL_GROUP=%s -PANDROID_CHANNEL=%s' % (channel_group, channel)

        mode_str = 'Debug' if mode == 'debug' else 'Release'
        # cmd = '"%s" --parallel --info assemble%s' % (gradle_path, mode_str)
        cmd = '"%s" --parallel --info assemble%s %s' % (gradle_path, mode_str, channel_str)
    ...
    def do_build_apk(self, channel, channel_group, mode, no_apk, no_sign, output_dir, custom_step_args, android_platform, compile_obj):
        ...
        if not no_apk:
            ...
            # build apk
            self.gradle_build_apk(channel, channel_group, mode, android_platform, compile_obj)

            # copy the apk to output dir
            # by zhangjm, apk path :proj.android/dists
            # if output_dir:
    ...
    def get_apk_info(self):
        ...
        gradle_cfg_path = os.path.join(android_app_path, 'build.gradle')
        ...
        if manifest_file is None:
            manifest_file = os.path.join(android_app_path, 'AndroidManifest.xml')
        else:
            manifest_file = os.path.join(android_app_path, manifest_file)            
        if package is None:
            # get package name from AndroidManifest.xml
            # package = self._xml_attr(manifest_path, 'AndroidManifest.xml', 'manifest', 'package')
            package = self._xml_attr_ex(manifest_file, 'manifest', 'package')

        # activity_name = self._xml_attr(manifest_path, 'AndroidManifest.xml', 'activity', 'android:name')
        activity_name = self._xml_attr_ex(manifest_file, 'activity', 'android:name')
