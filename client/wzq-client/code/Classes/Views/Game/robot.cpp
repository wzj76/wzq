...
Robot::Robot(unsigned int boardsize)
{
    m_boardsize = boardsize;
}

std::pair<int,int> Robot::GetkeySetchess(std::vector<std::vector<int>>&board)
{
    std::pair<int, int> ans;
    //机器人为黑棋并且是第一个棋子，下在中间
    if (m_steps == 0 && m_color_id ==BLACK_CHESS)
    {
       //放到中间
...
bool Robot::isOk(int x, int y)
{
    return x >= 0 && y >= 0 && x < m_boardsize && y < m_boardsize;
    
}
int Robot::GetAllScores(std::vector<std::vector<int>>&board,int x, int y)
{
    int ret = 0;
...
std::vector<int> Robot::GetScore1(std::vector<std::vector<int>>&board,int x, int y)
{
    int k = 0, t = 0, cnt = 5;
    while (cnt-- && isOk(x,y))
    {
...
std::vector<int> Robot::GetScore2(std::vector<std::vector<int>>&board,int x, int y)
{
    int k = 0, t = 0, cnt = 5;
    while (cnt-- && isOk(x, y))
    {
...
std::vector<int> Robot::GetScore3(std::vector<std::vector<int>>&board,int x, int y)
{
    int k = 0, t = 0, cnt = 5;
    while (cnt-- && isOk(x, y))
    {
...
std::vector<int> Robot::GetScore4(std::vector<std::vector<int>>&board,int x, int y)
{
    int k = 0, t = 0, cnt = 5;
    while (cnt-- && isOk(x, y))
    {
...
int Robot::CalculateScore(std::vector<int> nums)
{
    if (nums.empty())
    {
        return 0;
